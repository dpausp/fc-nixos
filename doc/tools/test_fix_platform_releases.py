...
class TestBannerGuard:
    ...
    def test_mixed_links_banner_survives_repl_phase(self, tmp_path):
        # Banner and a fixable link share the SAME URL: the finditer loop
        # skips the banner, and the re.sub repl must skip it again while
        # rewriting the plain occurrence — exercising the repl guard.
        page, _ = make_tree(tmp_path, BANNER_LINE + PLAIN_LINK_LINE)
        with structlog.testing.capture_logs() as logs:
            count, changes, errors, new_text = fpr.process_file(
                page, tmp_path, *fpr.build_indexes(tmp_path)
            )
        ...
        assert errors == []
        assert "[webgateway](web.md)" in new_text
        assert BANNER_LINE.strip() in new_text
        skips = [e for e in logs if e.get("event") == "banner-skipped"]
        assert len(skips) == 1
