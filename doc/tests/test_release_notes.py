...
def versions_file(tmp_path: Path) -> Path:
    path = tmp_path / "platform-versions.toml"
...
def hg(cwd: Path, *args: str) -> str:
    proc = subprocess.run(
        ["hg", *args], cwd=cwd, capture_output=True, text=True, check=False
    )
